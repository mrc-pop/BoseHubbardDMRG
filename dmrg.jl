...
function GetNumberVariance(psi, sites, j)
    # NOTA: anche se è un operatore di singolo sito, usando semplicemente op()
    # non c'era verso che funzionasse. Forse devo comunque definire l'operatore 
    # come MPO, in modo che agisca come l'identità (filo verticale) sugli  altri L-1 siti.
    n = OpSum()
    n += "N",j
    n = MPO(n, sites)
